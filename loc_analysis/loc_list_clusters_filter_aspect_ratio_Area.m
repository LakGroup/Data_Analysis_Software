function [data_filter_above,data_filter_below] = loc_list_clusters_filter_aspect_ratio_Area(data)
answer = inputdlg({'Major Axis to Minor Axis Aspect Ratio Threshold:','Enter Minimum Area per Cluster:'},'Input',[1 50],{'1.5','0.2'});
if isempty(answer)~=1
    min_aspect_ratio = str2double(answer{1});   
    min_area = str2double(answer{2}); 
    f = waitbar(0,'Filtering Based On Aspect Ratio + area...');
    for i=1:length(data)
        [data_filter_above{i},data_filter_below{i}] = filter_aspect_ratio(data{i},min_aspect_ratio,min_area);
        waitbar(i/length(data),f,['Filtering Based On Aspect Ratio + area...',num2str(i),'/',num2str(length(data))]);
    end 
    close(f)
...
function [data_to_send_above,data_to_send_below] = filter_aspect_ratio(data,min_aspect_ratio,min_area)
...
I = aspect_ratio<=min_aspect_ratio | (aspect_ratio>=min_aspect_ratio & areas>=min_area);
