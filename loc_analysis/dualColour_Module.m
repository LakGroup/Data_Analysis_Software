        ...
        if ~isempty(data_reference) && ~isempty(data_colocalization)
            ...
            if length(data_reference)==length(data_colocalization)
                ...
                for i = 1:length(data_reference)
                    ...
                    [data_localized{i}, data_not_localized{i}, Table{i}] = find_dual_colour(data_reference{i},data_colocalization{i},counter); % See inner function for more explanation.
...
function [data_localized, data_not_localized, Table] = find_dual_colour(data_reference,data_colocalization,counter_waitbar)
...
NonColocs = cellfun(@(x) x(:,1:2), NonColocs, 'UniformOutput', false);
...
for j = 1:size(NonColocs,2)
    ...
    [~,RotatedData] = pca(NonColocs{j}); % To rotate the data into the biggest possible major axis & smallest possible minor axis
    x_length = max(RotatedData(:,1))-min(RotatedData(:,1));
    y_length = max(RotatedData(:,2))-min(RotatedData(:,2));
