function data = load_microscopy_file_FramesCut()
...
if isequal(file_names,0)
    ...
    answer = inputdlg({'Frames Channel 0 (0 represents all frames):','Frames Channel 1:','Frames Channel 2:','Frames Channel 3:'},'Input',[1 50],{'0','0','0','0'});
    Frames(1) = str2double(answer{1});
    Frames(2) = str2double(answer{2});
    Frames(3) = str2double(answer{3});
    Frames(4) = str2double(answer{4});
    ...
    for i=1:length(file_names)
        ...
        try  
            data_read = load_microscopy_file_inside(fullfile(path,file_names{i}),file_type,Frames);            
...
function data_read = load_microscopy_file_inside(path,file_type,Frames)
...
elseif isequal(file_type,'csv')
    data_read = load_csv_inside(path,Frames);
end
...
function data_read = load_csv_inside(path,Frames)
table = csvread(path,1,0);
channels = unique(table(:,1));
for k = 1:length(channels)
    I = table(:,1)==channels(k);
    data_read{k} = table(I,[8 9]);
    FrameNumbers = table(I,2);
    UniqueFrames = unique(FrameNumbers);
    if Frames(k) ~= 0 && Frames(k) < numel(UniqueFrames)
        UniqueFrames = UniqueFrames(1:Frames(k));
        InsideOrNot = ismember(FrameNumbers,UniqueFrames);
        data_read{k} = data_read{k}(InsideOrNot,:);
    end
